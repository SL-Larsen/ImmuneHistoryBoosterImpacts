...
All codes and data necessary to produce simulation data, aggregate and analyse this data, and produce the figures for this paper are included. Please note that we have not included our simulation data as these files are extremely large and not storable by GitHub (e.g. individual variant-specific immune histories over time for thousands of simulations). These data can be made available upon reasonable request, but will not be stored in a public repository. 
