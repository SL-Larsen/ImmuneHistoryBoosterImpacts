COVID_vocBoost_hybrid_v4 <- function(t, x, parms) {
  ...
  with(as.list(params),{
    ...
    #Ecuador
    CM<-matrix(data=c(14.5, 4.31, 1.58, 5.53, 10.12, 1.94, 0.201, 0.193, 0.456), nrow=3)
    ...
    ## scale infect for people with immune history
    scale_immune <- 0.27
    ...
    ## SLL changed: adding this severity scalar that is uniform across immunity types (does not apply to)
    scalar_immunity <- 1-0.05
    ...
    #Recovered 
    dRch2 = gA_Ach*Ach2 + (1-((1-scalar_immunity)*rho_Ich))*gI_Ich*Ich2 - lambda_Rch*Rch2 - omega*Rch2
    dRcl2 = gA_Acl*Acl2 + (1-((1-scalar_immunity)*rho_Icl))*gI_Icl*Icl2 - lambda_Rcl*Rcl2 - omega*Rcl2 
    dRah2 = gA_Aah*Aah2 + (1-((1-scalar_immunity)*rho_Iah))*gI_Iah*Iah2 - lambda_Rah*Rah2 - omega*Rah2
    dRal2 = gA_Aal*Aal2 + (1-((1-scalar_immunity)*rho_Ial))*gI_Ial*Ial2 - lambda_Ral*Ral2 - omega*Ral2
    dReh2 = gA_Aeh*Aeh2 + (1-((1-scalar_immunity)*rho_Ieh))*gI_Ieh*Ieh2 - lambda_Reh*Reh2 - omega*Reh2
    dRel2 = gA_Ael*Ael2 + (1-((1-scalar_immunity)*rho_Iel))*gI_Iel*Iel2 - lambda_Rel*Rel2 - omega*Rel2
    
    dDch2 = rho_Ich*(1-scalar_immunity)*gI_Ich*Ich2
    dDcl2 = rho_Icl*(1-scalar_immunity)*gI_Icl*Icl2
    dDah2 = rho_Iah*(1-scalar_immunity)*gI_Iah*Iah2
    dDal2 = rho_Ial*(1-scalar_immunity)*gI_Ial*Ial2
    dDeh2 = rho_Ieh*(1-scalar_immunity)*gI_Ieh*Ieh2
    dDel2 = rho_Iel*(1-scalar_immunity)*gI_Iel*Iel2
    ...
    dRVch = gAV_AVch*AVch + (1-((1-scalar_immunity)*rho_Ich))*gIV_IVch*IVch + lambda_Rch*Rch - 
      omega*RVch - lambda_Rch2*RVch 
    dRVcl = gAV_AVcl*AVcl + (1-((1-scalar_immunity)*rho_Icl))*gIV_IVcl*IVcl + lambda_Rcl*Rcl - 
      omega*RVcl - lambda_Rcl2*RVcl 
    dRVah = gAV_AVah*AVah + (1-((1-scalar_immunity)*rho_Iah))*gIV_IVah*IVah + lambda_Rah*Rah - 
      omega*RVah - lambda_Rah2*RVah 
    dRVal = gAV_AVal*AVal + (1-((1-scalar_immunity)*rho_Ial))*gIV_IVal*IVal + lambda_Ral*Ral - 
      omega*RVal - lambda_Ral2*RVal 
    dRVeh = gAV_AVeh*AVeh + (1-((1-scalar_immunity)*rho_Ieh))*gIV_IVeh*IVeh + lambda_Reh*Reh - 
      omega*RVeh - lambda_Reh2*RVeh 
    dRVel = gAV_AVel*AVel + (1-((1-scalar_immunity)*rho_Iel))*gIV_IVel*IVel + lambda_Rel*Rel - 
      omega*RVel - lambda_Rel2*RVel 
    
    dDVch = (1-scalar_immunity)*rho_Ich*gIV_IVch*IVch
    dDVcl = (1-scalar_immunity)*rho_Icl*gIV_IVcl*IVcl
    dDVah = (1-scalar_immunity)*rho_Iah*gIV_IVah*IVah
    dDVal = (1-scalar_immunity)*rho_Ial*gIV_IVal*IVal
    dDVeh = (1-scalar_immunity)*rho_Ieh*gIV_IVeh*IVeh
    dDVel = (1-scalar_immunity)*rho_Iel*gIV_IVel*IVel
    ...
    #Recovered, vaccinated
    dRVch2 = gAV_AVch*AVch2 + (1-((1-scalar_immunity)*rho_Ich))*gIV_IVch*IVch2 - 
      omega*RVch2 + lambda_Rch*Rch2 - lambda_Rch2*RVch2 
    dRVcl2 = gAV_AVcl*AVcl2 + (1-((1-scalar_immunity)*rho_Icl))*gIV_IVcl*IVcl2 - 
      omega*RVcl2 + lambda_Rcl*Rcl2 - lambda_Rcl2*RVcl2
    dRVah2 = gAV_AVah*AVah2 + (1-((1-scalar_immunity)*rho_Iah))*gIV_IVah*IVah2 - 
      omega*RVah2 + lambda_Rah*Rah2 - lambda_Rah2*RVah2
    dRVal2 = gAV_AVal*AVal2 + (1-((1-scalar_immunity)*rho_Ial))*gIV_IVal*IVal2 - 
      omega*RVal2 + lambda_Ral*Ral2 - lambda_Ral2*RVal2
    dRVeh2=  gAV_AVeh*AVeh2 + (1-((1-scalar_immunity)*rho_Ieh))*gIV_IVeh*IVeh2 - 
      omega*RVeh2 + lambda_Reh*Reh2 - lambda_Reh2*RVeh2
    dRVel2 = gAV_AVel*AVel2 + (1-((1-scalar_immunity)*rho_Iel))*gIV_IVel*IVel2 - 
      omega*RVel2 + lambda_Rel*Rel2 - lambda_Rel2*RVel2
    
    dDVch2 = (1-scalar_immunity)*rho_Ich*gIV_IVch*IVch2
    dDVcl2 = (1-scalar_immunity)*rho_Icl*gIV_IVcl*IVcl2
    dDVah2 = (1-scalar_immunity)*rho_Iah*gIV_IVah*IVah2
    dDVal2 = (1-scalar_immunity)*rho_Ial*gIV_IVal*IVal2
    dDVeh2 = (1-scalar_immunity)*rho_Ieh*gIV_IVeh*IVeh2
    dDVel2 = (1-scalar_immunity)*rho_Iel*gIV_IVel*IVel2
    ...
    #Recovered, boosted
    dRBch = gAV_AVch*ABch + (1-((1-scalar_immunity)*rho_Ich))*gIV_IVch*IBch + 
      lambda_Rch2*RVch - omega*RBch 
    dRBcl = gAV_AVcl*ABcl + (1-((1-scalar_immunity)*rho_Icl))*gIV_IVcl*IBcl + 
      lambda_Rcl2*RVcl - omega*RBcl 
    dRBah = gAV_AVah*ABah + (1-((1-scalar_immunity)*rho_Iah))*gIV_IVah*IBah + 
      lambda_Rah2*RVah - omega*RBah 
    dRBal = gAV_AVal*ABal + (1-((1-scalar_immunity)*rho_Ial))*gIV_IVal*IBal + 
      lambda_Ral2*RVal - omega*RBal 
    dRBeh = gAV_AVeh*ABeh + (1-((1-scalar_immunity)*rho_Ieh))*gIV_IVeh*IBeh + 
      lambda_Reh2*RVeh - omega*RBeh 
    dRBel = gAV_AVel*ABel + (1-((1-scalar_immunity)*rho_Iel))*gIV_IVel*IBel + 
      lambda_Rel2*RVel - omega*RBel 
    
    #Deaths, boosted
    dDBch = (1-scalar_immunity)*rho_Ich*gIV_IVch*IBch
    dDBcl = (1-scalar_immunity)*rho_Icl*gIV_IVcl*IBcl
    dDBah = (1-scalar_immunity)*rho_Iah*gIV_IVah*IBah
    dDBal = (1-scalar_immunity)*rho_Ial*gIV_IVal*IBal
    dDBeh = (1-scalar_immunity)*rho_Ieh*gIV_IVeh*IBeh
    dDBel = (1-scalar_immunity)*rho_Iel*gIV_IVel*IBel
    ...
    #Recovered, boosted
    dRBch2 = gAV_AVch*ABch2 + (1-((1-scalar_immunity)*rho_Ich))*gIV_IVch*IBch2 + 
      lambda_Rch2*RVch2 - omega*RBch2 
    dRBcl2 = gAV_AVcl*ABcl2 + (1-((1-scalar_immunity)*rho_Icl))*gIV_IVcl*IBcl2 + 
      lambda_Rcl2*RVcl2 - omega*RBcl2 
    dRBah2 = gAV_AVah*ABah2 + (1-((1-scalar_immunity)*rho_Iah))*gIV_IVah*IBah2 + 
      lambda_Rah2*RVah2 - omega*RBah2  
    dRBal2 = gAV_AVal*ABal2 + (1-((1-scalar_immunity)*rho_Ial))*gIV_IVal*IBal2 + 
      lambda_Ral2*RVal2 - omega*RBal2  
    dRBeh2 = gAV_AVeh*ABeh2 + (1-((1-scalar_immunity)*rho_Ieh))*gIV_IVeh*IBeh2 + 
      lambda_Reh2*RVeh2 - omega*RBeh2  
    dRBel2 = gAV_AVel*ABel2 + (1-((1-scalar_immunity)*rho_Iel))*gIV_IVel*IBel2 + 
      lambda_Rel2*RVel2 - omega*RBel2  
    
    #Deaths, boosted
    dDBch2 = (1-scalar_immunity)*rho_Ich*gIV_IVch*IBch2
    dDBcl2 = (1-scalar_immunity)*rho_Icl*gIV_IVcl*IBcl2
    dDBah2 = (1-scalar_immunity)*rho_Iah*gIV_IVah*IBah2
    dDBal2 = (1-scalar_immunity)*rho_Ial*gIV_IVal*IBal2
    dDBeh2 = (1-scalar_immunity)*rho_Ieh*gIV_IVeh*IBeh2
    dDBel2 = (1-scalar_immunity)*rho_Iel*gIV_IVel*IBel2
