#This version is for faster omicron wave which leads to more immunity
...
sweep_fun <- function(t = t){
  ...
  sd0=0.7088
  wanevar=c(1/300)
  #pboost_h=c(0, 0.03457, 0.1423)
  #pboost_l=c(0, 0.0105, 0.0561)
  boost.start=c(0, 60)
  ...
  imm<-data.frame(cp=cp, base.imm=base.imm, sd0=sd0)
  ...
  check0<-as.data.frame(expand.grid(boost.child=boost.child, cvax=cvax, boost.adult=boost.adult, max.boost_h=max.boost_h, 
                                    ...
                                    wanevar=wanevar, child.max=child.max, cp=cp, base.imm=base.imm, sd0=sd0, relax=relax, boost.start=boost.start,
