...
source("model_setting_ecuadorsweep_010424.R")
tmax<-195
num_sweep <- sweep_fun(t = (0:195))$num_sweep
...
model_sims <- function(i){
  ...
  dist<-c(0.41*lses, 0.41*hses, 0.526*lses, 0.526*hses, 0.064*lses, 0.064*hses)
  
  start.Ns<-dist*(15735139)
  distI<-c(0.534*lses, 0.534*hses, 0.378*lses, 0.378*hses, 0.088*lses, 0.088*hses) #
  distH<-c(0.04*lses, 0.04*hses, 0.566*lses, 0.566*hses, 0.394*lses, 0.394*hses) #Age distribution for hospitalizations, 
  ...
  report<-(0.189) #Reporting rate, lower bound on reporting with upper bound on CI
  ...
  #start.Is<-distI*(0.5*15634)*(1/report) #To get current Is, did sum(I.new, t=0:7) with rounding 
  start.Is<-distI*(1798)*(1/report)
  ...
  #sum(sum(start.Is), sum(start.As), sum(start.Es))/sum(start.Ns)
...
save.image('../../../../Data/ODE outputs SA - ABM beta/sweep_010424_ecuadorboost_Evax.RData')
...
saveRDS(end, '../../../../Data/ODE outputs SA - ABM beta/endfile_sweep010424_ecuadorboost_Evax.RDS')

#For debugging to check population size
#t0<-subset(model_out_dat, model_out_dat$time==0)
#for(i in 1:nrow(t0)){
#  t0$startsum<-sum(t0[i,c(2:31, 38:67, 74:103, 110:139, 146:175, 182:211)])
#}
