...
## Boosting start time 
t_boost_start = grid['t_boost_start'][indexID-1]
...
## vaccines
    ...
    for j in range(to_vax_L):
      if len(unVaxLow)>0:
        ...
        numVaccinatedL = numVaccinatedL + 1
## boosters
    if boost == "yes" and t > t_boost_start:
      if boost_shape == "functional":
        to_boost_H = vaxTrend(Vm_H_boost, Wh_H_boost, h_H, countT-t_boost_start, 1, nonchild*numVaccinatedH)
        to_boost_L = vaxTrend(Vm_L_boost, Wh_L_boost, h_L, countT-t_boost_start, 1, nonchild*numVaccinatedL)
      else:
