...
heat_map_data <- serotypes %>% select(history, WT, Delta, Omicron, Omicron_star) %>% 
  mutate(history= gsub("_star", "*", history),
         history = gsub("_", " + ", history)) %>% 
  pivot_longer(WT:Omicron_star, names_to = "wave", values_to = "neg_protection") %>% 
  mutate(wave = gsub("_star", "*", wave)) %>% 
  mutate(wave = fct_relevel(wave, c("WT", "Delta", "Omicron", "Omicron*"))) %>%
...
plot2A <- data_protection %>% mutate(history = fct_relevel(history, 
...
plot2B <- dat2B %>%
  ...
  ggtitle("Infections during Omicron* - History specific model") + #, "Variant-specific model") +
...
plot3A <- ODE_ts %>% 
  ...
  #scale_x_continuous("Time (months)", expand = c(0,0.5), breaks = c(30,32,34,36)) +
      scale_x_date("Date", expand = c(0,0), date_breaks = "1 month", date_labels =  "%b %Y") + 
  ggtitle("Infections during Omicron* - Hybrid immunity model") + 
  theme(axis.text = element_text(size = 11, color = "black"),
        axis.text.x=element_text(angle=60, hjust=1),
        axis.title = element_text(size = 12, color = "black"),
        axis.line = element_line(color = "black"),
